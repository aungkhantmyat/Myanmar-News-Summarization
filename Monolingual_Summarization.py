...
async def load_model_and_tokenizer():
    tokenizer = AutoTokenizer.from_pretrained("tym24/mt5-summarize-ja")
    model = AutoModelForSeq2SeqLM.from_pretrained("tym24/mt5-summarize-ja")
...
with st.sidebar:
    selected = option_menu(
        ...
        options=["Monolingual Summarization", "Evaluation"],
        icons=["lightbulb-fill","bar-chart-line-fill"],
        menu_icon="file-earmark-text-fill",
        default_index=0
    )

if selected == "Monolingual Summarization":
    # layout ----------------
    # title and desc
    st.title("Myanmar News Summarization with Single-language Model")
    st.markdown(
